...
template <class L, class R> auto create_operator_error(const std::string& str) -> Error {
  std::string e = str;
  e += " not permitted (";
  e += typeid(L).name();
  e += ",";
...
auto add(VMType& lhs, VMType& rhs) -> ResultOr<VMType> {
  if (std::holds_alternative<VMPrimitive>(lhs) && std::holds_alternative<VMPrimitive>(rhs)) {
    ...
    return std::visit(
        [](const auto& lhv, const auto& rhv) -> ResultOr<VMType> {
          ...
          if constexpr (std::is_arithmetic_v<L> && std::is_arithmetic_v<R>) {
            ...
            return VMType{
                VMPrimitive{static_cast<CommonType>(lhv) + static_cast<CommonType>(rhv)}}; // Numerische Addition
  ...
  if (std::holds_alternative<VMPrimitive>(value)) {
    return std::visit(
        [](const auto& v) -> ResultOr<std::string> {
          ...
          return err("to_string not permitted on type " + std::string(typeid(L).name()));
        },
...
template <class L, class R> struct is_vm_primitive_adr_cmp {
  static constexpr bool value = std::is_arithmetic_v<L> && std::is_same_v<R, VMAddress>;
};
template <class L, class R> constexpr bool is_vm_primitive_adr_cmp_v = is_vm_primitive_adr_cmp<L, R>::value;

template <class Func> auto vm_primitive_cmp(const auto& _lhs, const auto& _rhs, Func&& op) -> bool {
  ...
  if constexpr (is_vm_primitive_adr_cmp_v<L, R>) {
    return op(static_cast<std::size_t>(_lhs), static_cast<VMAddress>(_rhs).get());
  }
  if constexpr (is_vm_primitive_adr_cmp_v<R, L>) {
    return op(static_cast<VMAddress>(_lhs).get(), static_cast<std::size_t>(_rhs));
  }
  if constexpr (std::is_same_v<R, std::string> && std::is_same_v<L, std::string>) {
    return op(_lhs, _rhs);
  }
  return false;
}
...
auto to_string(const VMStructTypes& type) -> ResultOr<std::string> {
  ...
  return std::visit(overloaded{[&](const VMPrimitive& t) -> ResultOr<std::string> {
                                 VMType tt = t;
                                 return to_string(tt);
                               },
...
auto get_data_ptr_and_size(const VMType& type) -> std::pair<const void*, std::size_t> {
  if (auto prim = std::get_if<VMPrimitive>(&type)) {
    return std::visit(
        overloaded{
            [](const std::string& str) -> std::pair<const void*, std::size_t> { return {str.data(), str.size()}; },
            [](const VMAddress& adr) -> std::pair<const void*, std::size_t> { return {&adr, sizeof(VMAddress)}; },
            [](const auto& val) -> std::pair<const void*, std::size_t> { return {&val, sizeof(val)}; }},
        *prim);
  } else if (auto adr = std::get_if<VMAddress>(&type)) {
    return {adr, sizeof(VMAddress)};
  } else if (auto vmStruct = std::get_if<VMStruct>(&type)) {
    return {vmStruct, vmStruct->size()};
  }
  return {nullptr, 0};
}
  ...
  if (auto prim = std::get_if<VMPrimitive>(&type)) {
    return std::visit(
        overloaded{
            ...
            [](const auto& val) -> std::pair<const void*, std::size_t> { return {&val, sizeof(val)}; }},
        *prim);
  } else if (auto adr = std::get_if<VMStructPtr>(&type)) {
    return {adr, sizeof(VMAddress)};
  }
  return {nullptr, 0};
}
