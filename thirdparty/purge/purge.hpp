...
class ptester {
public:
  void test(const std::string &t, int line_no, bool res,
...
#define SIMPLE_TEST_CASE(TEST_NAME)                                            \
  ...
  namespace {                                                                  \
  purge::test_registry PURGE_UNIQUE_NAME(TEST_NAME)(TEST_NAME,                 \
...
// METHOD_TEST_CASE(TEST_CLASS_METHOD,NumberClass,is_number,(false,'a'), (
// true,'1'),(false,'b'));
#define METHODE_TEST_CASE(TEST_NAME, CLASS, CONSTRUCT_PARAMETER, METHODE,      \
                          TUPLE, ...)                                          \
  namespace {                                                                  \
  purge::test_registry PURGE_UNIQUE_NAME(TEST_NAME)(                           \
      []() {                                                                   \
  ...
  purge::test_registry PURGE_UNIQUE_NAME(TEST_NAME)(                           \
      []() {                                                                   \
        ...
        for (auto &x : vec) {                                                  \
          ...
          std::apply(                                                          \
              [&correct_test, &res](auto &&...args) {                          \
                correct_test = (res == TEST_FUNC(args...));                    \
              },                                                               \
              parameter);                                                      \
...
#define REQUIRE(X) ptester().test(#X, __LINE__, X, __FUNCTION__)
