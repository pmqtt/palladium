...
enum class TEST_STATE { OK, FAILED, SEGMENTATION_FAULT };
...
std::string to_string(purge::TEST_STATE t) {
  static std::map<purge::TEST_STATE, std::string> map_test_state_to_string = {
      ...
      {purge::TEST_STATE::SEGMENTATION_FAULT,
       "\033[1;35m Segmentation fault\033[0m"}};
...
public:
  ...
  bool all_tests_ok() {
    for (auto &x : entries) {
      for (auto y : x.test_items) {
        if (y.result_state != TEST_STATE::OK) {
          return false;
        }
      }
    }
  ...
  void add_entry(const log_book_entry &entry) { entries.push_back(entry); }
  ...
  std::stringstream generate_output() {
    std::size_t first_len = std::string("Test").length();
    std::size_t second_len = std::string(" Check |").length();
    std::size_t third_len = std::string(" Result").length();
    for (auto &x : entries) {
    ...
    // output<<"Test"<<std::setw(tLength+first_len)<<"| Check
    // "<<std::setw(second_len) <<" | Result\n";
    // output<<std::setfill('=')<<std::setw(first_len + second_len +
    // third_len)<<"\n"; output<<std::setfill(' ');
    output << std::setw(first_len + second_len + third_len + 20)
           << std::setfill('=') << "\n";
    output << std::setfill(' ');
    output << "\033[1m"
           << std::setw((first_len + second_len + third_len + 20) / 2)
           << "PURGE RESULT\033[0m"
           << "\n";
    output << std::setfill('=');
    output << std::setw(first_len + second_len + third_len + 20)
    ...
    for (auto &x : entries) {
      ...
      output << std::setfill(' ');
      for (auto &y : x.test_items) {
        output << std::setw(first_len + tLength - 2) << "| "
               << y.check_description
               << std::setw(second_len - y.check_description.length() + 3)
               << "|" << to_string(y.result_state) << "\n";
      }
      output << "\n";
    }
    return output;
